...
**Tech Stack:** Python 3.11+, `typer` (CLI), `rich` (output), `Pillow`+`piexif` (EXIF/jpeg/tiff), `rawpy` (CR3 previews), `opencv-python`+`numpy` (sharpness/exposure), `imagehash` (perceptual hash), `anthropic` (vision), stdlib `sqlite3`, `pytest`.
